...
Unit tests may be run using the `make test` command. More targeted unit tests may be run with `UNIT_TEST=true go test -v -tags=unit_test`. You must set `UNIT_TEST=true` and add the `unit_test` tag, otherwise tests will be skipped or you will hit initialization errors.
