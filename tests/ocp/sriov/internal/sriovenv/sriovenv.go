...
var (
	...
	// NetConfig provides access to SR-IOV test configuration parameters.
	NetConfig *tsparams.NetworkConfig
)
...
func init() {
	NetConfig = tsparams.GetNetworkConfig()
...
func IsSriovDeployed(apiClient *clients.Settings, config *tsparams.NetworkConfig) error {
	...
	// Check for operator pods using eco-goinfra
	podList, err := pod.List(apiClient, config.SriovOperatorNamespace, metav1.ListOptions{})
	if err != nil {
	...
	for _, p := range podList {
		if p.Definition.Status.Phase == corev1.PodRunning {
			hasRunningPod = true
			glog.V(90).Infof("Found running SR-IOV operator pod %q in namespace %q", p.Definition.Name, p.Definition.Namespace)
			break
		}
	}
	...
	glog.V(90).Infof("SR-IOV operator is deployed and ready in namespace %q (operator_pods: %d, sriov_networks: %d)",
		config.SriovOperatorNamespace, len(podList), len(sriovNetworks))
...
func VerifyVFResourcesAvailable(apiClient *clients.Settings, config *tsparams.NetworkConfig, resourceName string) (bool, error) {
	if apiClient == nil {
		return false, fmt.Errorf("API client is nil, cannot verify VF resources")
	}

	// Get all worker nodes
	workerNodes, err := nodes.List(apiClient, metav1.ListOptions{LabelSelector: config.WorkerLabel})
	if err != nil {
...
func CreateSriovNetwork(apiClient *clients.Settings, config *SriovNetworkConfig, timeout time.Duration) error {
	...
	err = wait.PollUntilContextTimeout(
		...
		true,
		func(ctx context.Context) (bool, error) {
			// We need NetConfig for this, but we can get it from the package
			available, err := VerifyVFResourcesAvailable(apiClient, NetConfig, config.ResourceName)
			if err != nil {
				return false, err
			}
...
func CheckSriovOperatorStatus(apiClient *clients.Settings, config *tsparams.NetworkConfig) error {
	glog.V(90).Infof("Checking SR-IOV operator status in namespace %q", config.SriovOperatorNamespace)
	return IsSriovDeployed(apiClient, config)
}

// WaitForSriovPolicyReady waits for SR-IOV policy to be ready and MCP to be stable
func WaitForSriovPolicyReady(apiClient *clients.Settings, config *tsparams.NetworkConfig, timeout time.Duration) error {
	glog.V(90).Infof("Waiting for SR-IOV policy to be ready (timeout: %v)", timeout)
	return WaitForSriovAndMCPStable(
		apiClient, timeout, 30*time.Second, config.CnfMcpLabel, config.SriovOperatorNamespace)
}
...
func InitVF(apiClient *clients.Settings, config *tsparams.NetworkConfig, name, deviceID, interfaceName, vendor, sriovOpNs string, vfNum int, workerNodes []*nodes.Builder) (bool, error) {
	...
	for _, node := range workerNodes {
		...
		err = WaitForSriovAndMCPStable(
			apiClient, 20*time.Minute, 30*time.Second, config.CnfMcpLabel, sriovOpNs)
		if err != nil {
...
func CreateTestPod(apiClient *clients.Settings, config *tsparams.NetworkConfig, name, namespace, networkName, ipAddress, macAddress string) (*pod.Builder, error) {
...
func CheckVFStatusWithPassTraffic(apiClient *clients.Settings, config *tsparams.NetworkConfig, networkName, interfaceName, namespace, description string, timeout time.Duration) error {
...
func InitDpdkVF(apiClient *clients.Settings, config *tsparams.NetworkConfig, name, deviceID, interfaceName, vendor, sriovOpNs string, vfNum int, workerNodes []*nodes.Builder) (bool, error) {
...
func GetPciAddress(apiClient *clients.Settings, config *tsparams.NetworkConfig, namespace, podName, policyName string) (string, error) {
	glog.V(90).Infof("Getting PCI address for pod %q in namespace %q (policy: %q)", podName, namespace, policyName)

	podBuilder := pod.NewBuilder(apiClient, podName, namespace, config.CnfNetTestContainer)
...
func CreateDpdkTestPod(apiClient *clients.Settings, config *tsparams.NetworkConfig, name, namespace, networkName string) (*pod.Builder, error) {
