...
var _ = Describe(
	...
	func() {
		var (
			...
			sriovOpNs            = sriovenv.NetConfig.SriovOperatorNamespace
		...
		BeforeAll(func() {
			...
			err := sriovenv.CheckSriovOperatorStatus(sriovenv.APIClient, sriovenv.NetConfig)
			Expect(err).ToNot(HaveOccurred(), "SR-IOV operator is not running")

			By("Discovering worker nodes")
			workerNodes, err = nodes.List(sriovenv.APIClient,
				metav1.ListOptions{LabelSelector: labels.Set(sriovenv.NetConfig.WorkerLabelMap).String()})
		...
		AfterAll(func() {
			...
			for _, item := range testData {
				err := sriovenv.RemoveSriovPolicy(sriovenv.APIClient, item.Name, sriovOpNs, tsparams.DefaultTimeout)
				if err != nil {
			...
			err := sriovenv.WaitForSriovPolicyReady(sriovenv.APIClient, sriovenv.NetConfig, tsparams.DefaultTimeout)
		...
		It("SR-IOV VF with spoof checking enabled", reportxml.ID("25959"), func() {
			...
			for _, data := range testData {
				...
				result, err := sriovenv.InitVF(sriovenv.APIClient, sriovenv.NetConfig, data.Name, data.DeviceID, data.InterfaceName, data.Vendor, sriovOpNs, vfNum, workerNodes)
				...
				func() {
					...
					nsBuilder := namespace.NewBuilder(sriovenv.APIClient, ns1)
					...
					err = sriovenv.CreateSriovNetwork(sriovenv.APIClient, networkConfig, tsparams.WaitTimeout)
					...
					DeferCleanup(func() {
						...
						err := sriovenv.RemoveSriovNetwork(sriovenv.APIClient, networkName, sriovOpNs, tsparams.DefaultTimeout)
						Expect(err).ToNot(HaveOccurred(), "Failed to remove SR-IOV network %q", networkName)
					})

					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, "spoof checking on", tsparams.PodReadyTimeout)
		...
		It("SR-IOV VF with spoof checking disabled", reportxml.ID("70820"), func() {
			...
			for _, data := range testData {
				...
				func() {
					...
					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, "spoof checking off", tsparams.PodReadyTimeout)
					if err != nil && strings.Contains(err.Error(), "NO-CARRIER") {
						Skip("Interface has NO-CARRIER status - skipping connectivity test for interface without physical connection")
					}
					Expect(err).ToNot(HaveOccurred(), "VF status verification failed")
		...
		It("SR-IOV VF with trust disabled", reportxml.ID("25960"), func() {
			...
			for _, data := range testData {
				...
				func() {
					...
					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, "trust off", tsparams.PodReadyTimeout)
					if err != nil && strings.Contains(err.Error(), "NO-CARRIER") {
						Skip("Interface has NO-CARRIER status - skipping connectivity test for interface without physical connection")
					}
					Expect(err).ToNot(HaveOccurred(), "VF status verification failed")
		...
		It("SR-IOV VF with trust enabled", reportxml.ID("70821"), func() {
			...
			for _, data := range testData {
				...
				func() {
					...
					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, "trust on", tsparams.PodReadyTimeout)
					if err != nil && strings.Contains(err.Error(), "NO-CARRIER") {
						Skip("Interface has NO-CARRIER status - skipping connectivity test for interface without physical connection")
					}
					Expect(err).ToNot(HaveOccurred(), "VF status verification failed")
		...
		It("SR-IOV VF with VLAN and rate limiting configuration", reportxml.ID("25963"), func() {
			...
			for _, data := range testData {
				...
				func() {
					...
					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, "vlan 100, qos 2, tx rate 100 (Mbps), max_tx_rate 100Mbps, min_tx_rate 40Mbps", tsparams.PodReadyTimeout)
					if err != nil && strings.Contains(err.Error(), "NO-CARRIER") {
						Skip("Interface has NO-CARRIER status - skipping connectivity test for interface without physical connection")
					}
					Expect(err).ToNot(HaveOccurred(), "VF status verification failed")
		...
		It("SR-IOV VF with auto link state", reportxml.ID("25961"), func() {
			...
			for _, data := range testData {
				...
				func() {
					...
					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, "link-state auto", tsparams.PodReadyTimeout)
					if err != nil && strings.Contains(err.Error(), "NO-CARRIER") {
						Skip("Interface has NO-CARRIER status - skipping connectivity test for interface without physical connection")
					}
					Expect(err).ToNot(HaveOccurred(), "VF status verification failed")
		...
		It("SR-IOV VF with enabled link state", reportxml.ID("71006"), func() {
			...
			for _, data := range testData {
				...
				func() {
					...
					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, "link-state enable", tsparams.PodReadyTimeout)
					if err != nil && strings.Contains(err.Error(), "NO-CARRIER") {
						Skip("Interface has NO-CARRIER status - skipping connectivity test for interface without physical connection")
					}
					Expect(err).ToNot(HaveOccurred(), "VF status verification failed")
		...
		It("MTU configuration for SR-IOV policy", reportxml.ID("69646"), func() {
			...
			for _, data := range testData {
				...
				err = sriovenv.UpdateSriovPolicyMTU(sriovenv.APIClient, data.Name, sriovOpNs, mtuValue)
				...
				func() {
					...
					By("Verifying VF status with pass traffic")
					err = sriovenv.CheckVFStatusWithPassTraffic(sriovenv.APIClient, sriovenv.NetConfig, networkName, data.InterfaceName, ns1, fmt.Sprintf("mtu %d", mtuValue), tsparams.PodReadyTimeout)
					if err != nil && strings.Contains(err.Error(), "NO-CARRIER") {
						Skip("Interface has NO-CARRIER status - skipping connectivity test for interface without physical connection")
					}
					Expect(err).ToNot(HaveOccurred(), "VF status verification failed")
		...
		It("DPDK SR-IOV VF functionality validation", reportxml.ID("69582"), func() {
			...
			for _, data := range testData {
				...
				result, err := sriovenv.InitDpdkVF(sriovenv.APIClient, sriovenv.NetConfig, data.Name, data.DeviceID, data.InterfaceName, data.Vendor, sriovOpNs, vfNum, workerNodes)
				...
				func() {
					...
					_, err = sriovenv.CreateDpdkTestPod(sriovenv.APIClient, sriovenv.NetConfig, "sriovdpdk", ns1, networkName)
					...
					err = sriovenv.WaitForPodWithLabelReady(sriovenv.APIClient, ns1, "name=sriov-dpdk", tsparams.PodReadyTimeout)
					...
					pciAddress, err := sriovenv.GetPciAddress(sriovenv.APIClient, sriovenv.NetConfig, ns1, "sriovdpdk", policyName)
					...
					podBuilder, err := pod.Pull(sriovenv.APIClient, "sriovdpdk", ns1)
