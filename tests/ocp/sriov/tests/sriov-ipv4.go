...
var _ = Describe("SR-IOV IPv4", Ordered, Label(tsparams.LabelSriovIPv4), ContinueOnFailure, func() {
	const (
	...
	BeforeAll(func() {
		...
		clusterIPFamily, err := sriovocpenv.GetClusterIPFamily(APIClient)
		Expect(err).ToNot(HaveOccurred(), "Failed to detect cluster IP family")

		if !sriovocpenv.ClusterSupportsIPv4(clusterIPFamily) {
		...
		workerNodeList, err = nodes.List(
			APIClient, metav1.ListOptions{LabelSelector: labels.Set(SriovOcpConfig.WorkerLabelMap).String()})
		...
		sriovInterfaces, err = SriovOcpConfig.GetSriovInterfaces(2)
		Expect(err).ToNot(HaveOccurred(), "Failed to retrieve SR-IOV interfaces for testing")

		sriovInterfacePF1 = sriovInterfaces[0]
		sriovInterfacePF2 = sriovInterfaces[1]

		By("Enabling SCTP on worker nodes if it is not already enabled")
		Expect(sriovenv.EnableSCTPOnWorkerNodes()).ToNot(HaveOccurred(),
			"Failed to enable SCTP on worker nodes")
	...
	Context("Same Node Same PF", func() {
		...
		It("Verify SR-IOV IPv4 connectivity with Static IPAM and Static MAC", reportxml.ID("87398"), func() {
			...
			clientMTU500, _, err = sriovenv.CreatePodPair(
				...
				sriovenv.RemovePrefix(tsparams.ServerIPv4IPAddress),
			...
			clientMTU9000, _, err = sriovenv.CreatePodPair(
				...
				sriovenv.RemovePrefix(tsparams.ServerIPv4IPAddress2), tsparams.ClientMacAddress2, tsparams.ServerMacAddress2,
			...
			err = sriovenv.RunTrafficTestsForBothMTUs(clientMTU500, clientMTU9000,
				sriovenv.RemovePrefix(tsparams.ServerIPv4IPAddress),
				sriovenv.RemovePrefix(tsparams.ServerIPv4IPAddress2),
				mtu500, mtu9000)
		...
		It("Verify SR-IOV IPv4 connectivity with Whereabouts IPAM, Dynamic MAC, and VLAN",
			reportxml.ID("87399"), func() {
				if SriovOcpConfig.VLAN == "" {
					Skip("ECO_OCP_SRIOV_VLAN is not set")
				}

				By("Creating VLAN SR-IOV networks")

				vlan, err := SriovOcpConfig.GetVLAN()
				Expect(err).ToNot(HaveOccurred(), "Failed to get VLAN ID from ECO_OCP_SRIOV_VLAN")
		...
		It("Verify SR-IOV IPv4 connectivity between different PFs on same node with Static IPAM and Static MAC",
			reportxml.ID("87400"), func() {
				...
				clientMTU500, _, err = sriovenv.CreatePodPair(
					...
					sriovenv.RemovePrefix(tsparams.ServerIPv4IPAddress), tsparams.ClientMacAddress, tsparams.ServerMacAddress,
					[]string{tsparams.ClientIPv4IPAddress}, []string{tsparams.ServerIPv4IPAddress},
					mtu500)
				Expect(err).ToNot(HaveOccurred(), "Failed to create pods for MTU 500")
