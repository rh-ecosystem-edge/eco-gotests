	...
	ContinueOnFailure, func() {
		...
		BeforeAll(func() {
			...
			_, err = sriov.NewPolicyBuilder(
				...
				"resource1",
				6,
				[]string{sriovInterfacesUnderTest[0]}, SriovOcpConfig.WorkerLabelMap).
			...
			_, err = sriov.NewPolicyBuilder(
				...
				"resource2",
				6,
				[]string{sriovInterfacesUnderTest[1]}, SriovOcpConfig.WorkerLabelMap).
				WithDevType("netdevice").Create()
			Expect(err).ToNot(HaveOccurred(), "Failed to configure SR-IOV policy")
		...
		AfterEach(func() {
			...
			err := namespace.NewBuilder(APIClient, tsparams.TestNamespaceName1).CleanObjects(
				tsparams.DefaultTimeout, sriov.GetSriovNetworksGVR(), nad.GetGVR())
