...
func checkPodIPv6Ready(podObj *pod.Builder, interfaceName, ipv6Addr string) (bool, error) {
	...
	if err != nil {
		glog.V(rdscoreparams.RDSCoreLogLevel).Infof("Failed to execute ip addr command in pod %q: %v",
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, ipv6Addr) && strings.Contains(line, "inet6") {
			...
			if strings.Contains(line, "dadfailed") {
				glog.V(rdscoreparams.RDSCoreLogLevel).Infof("IPv6 address %s DAD failed for pod %q: %s",
					ipv6Addr, podObj.Object.Name, strings.TrimSpace(line))
			...
			glog.V(rdscoreparams.RDSCoreLogLevel).Infof("IPv6 address %s is ready (not tentative) for pod %q",
	...
	if err := scanner.Err(); err != nil {
		glog.V(rdscoreparams.RDSCoreLogLevel).Infof("Error scanning output for IPv6 address %s in pod %q: %v",
	...
	glog.V(rdscoreparams.RDSCoreLogLevel).Infof("IPv6 address %s not found in output for pod %q",
		ipv6Addr, podObj.Object.Name)
...
func getActivePods(podLabel, namespace string) []*pod.Builder {
	...
	for _, _pod := range pods {
		...
		case corev1.PodPending, corev1.PodSucceeded, corev1.PodFailed, corev1.PodUnknown:
			...
			glog.V(rdscoreparams.RDSCoreLogLevel).Infof("Pod %q is not running, dumping status details:",
				_pod.Object.Name)

			// Log pod conditions
			for _, condition := range _pod.Object.Status.Conditions {
				glog.V(rdscoreparams.RDSCoreLogLevel).Infof("  Condition: Type=%s, Status=%s, Reason=%s, Message=%s",
					condition.Type, condition.Status, condition.Reason, condition.Message)
			}

			// Log container statuses
			for _, containerStatus := range _pod.Object.Status.ContainerStatuses {
				glog.V(rdscoreparams.RDSCoreLogLevel).Infof("  ContainerStatus: Name=%s, Ready=%t, RestartCount=%d",
					containerStatus.Name, containerStatus.Ready, containerStatus.RestartCount)

				if containerStatus.State.Waiting != nil {
					glog.V(rdscoreparams.RDSCoreLogLevel).Infof("    Waiting: Reason=%s, Message=%s",
						containerStatus.State.Waiting.Reason, containerStatus.State.Waiting.Message)
				}

				if containerStatus.State.Terminated != nil {
					glog.V(rdscoreparams.RDSCoreLogLevel).Infof("    Terminated: Reason=%s, Message=%s, ExitCode=%d",
			...
			for _, initContainerStatus := range _pod.Object.Status.InitContainerStatuses {
				glog.V(rdscoreparams.RDSCoreLogLevel).Infof("  InitContainerStatus: Name=%s, Ready=%t, RestartCount=%d",
			...
			if _pod.Object.Status.NominatedNodeName != "" {
				glog.V(rdscoreparams.RDSCoreLogLevel).Infof("  NominatedNodeName: %s", _pod.Object.Status.NominatedNodeName)
			}

			if _pod.Object.Spec.NodeName == "" {
				glog.V(rdscoreparams.RDSCoreLogLevel).Infof("  Pod has not been scheduled to a node yet")
			} else {
				glog.V(rdscoreparams.RDSCoreLogLevel).Infof("  Pod scheduled to node: %s", _pod.Object.Spec.NodeName)
			}
		}
	}
