...
func checkIPv6AddressState(output, ipv6Addr string) (bool, error) {
	if ipv6Addr == "" {
		glog.V(100).Infof("Empty IPv6 address provided")
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, ipv6Addr) && strings.Contains(line, "inet6") {
			...
			if strings.Contains(line, "tentative") {
				glog.V(100).Infof("IPv6 address %s is in tentative state (DAD in progress): %s",
			...
			if strings.Contains(line, "dadfailed") {
				glog.V(100).Infof("IPv6 address %s DAD failed: %s",
					ipv6Addr, strings.TrimSpace(line))
			...
			glog.V(100).Infof("IPv6 address %s is ready (not tentative): %s",
				ipv6Addr, strings.TrimSpace(line))
	...
	if err := scanner.Err(); err != nil {
		glog.V(100).Infof("Error scanning output for IPv6 address %s: %v", ipv6Addr, err)
	...
	glog.V(100).Infof("IPv6 address %s not found in output", ipv6Addr)
	...
	err := wait.PollUntilContextTimeout(
		...
		func(ctx context.Context) (bool, error) {
			...
			if ipv4Addr != "" {
				...
				if err != nil {
					glog.V(100).Infof("Failed to parse output due to %v, retrying...", err)
			...
			if ipv6Addr != "" {
				...
				if err != nil {
					glog.V(100).Infof("IPv6 address %s DAD failed for pod %s in namespace %s: %v",
				...
				if !ipv6Ready {
					glog.V(100).Infof("IPv6 address %s not ready yet (tentative state) for pod %s in namespace %s, retrying...",
						ipv6Addr, podObj.Definition.Name, podObj.Definition.Namespace)

					return false, nil
				}

				glog.V(100).Infof("IPv6 address %s is ready and not in tentative state", ipv6Addr)
			}
...
func waitForPodLevelBondNodesSriovSync() error {
	...
	for _, nodeName := range nodeNames {
		glog.V(rdscoreparams.RDSCoreLogLevel).Infof("Checking SRIOVNetworkNodeState sync status on node %q", nodeName)
