...
const (
	...
	// imageSignatureCleanupTimeout is the overall time allowed to remove a workload during cleanup.
	imageSignatureCleanupTimeout = 5 * time.Minute
	// imageSignatureDeleteTimeout is the per-attempt deployment deletion timeout.
	imageSignatureDeleteTimeout = 2 * time.Minute
)
...
func verifyImageSignatureDeployment(ctx SpecContext, deployName, deployNs, image string, shouldSucceed bool) {
	...
	// Register cleanup before creating the workload, so the deployment is removed even when the
	// creation itself times out or the subsequent assertions fail.
	DeferCleanup(cleanupImageSignatureDeployment, deployName, deployNs)
...
// cleanupImageSignatureDeployment removes the workload created by an image signature scenario,
// including the pods left in CreateContainerError by the negative scenarios. Deletion is retried
// so that transient API errors during teardown do not fail an otherwise passing spec.
func cleanupImageSignatureDeployment(ctx SpecContext, deployName, deployNs string) {
	By(fmt.Sprintf("Removing image signature deployment %q from %q ns", deployName, deployNs))

	Eventually(func() error {
		deploy, err := deployment.Pull(APIClient, deployName, deployNs)
		if err != nil {
			klog.V(rdscoreparams.RDSCoreLogLevel).Infof(
				"Deployment %q not found in %q ns: %v", deployName, deployNs, err)

			return nil
		}

		return deploy.DeleteAndWait(imageSignatureDeleteTimeout)
	}).WithContext(ctx).WithPolling(10*time.Second).WithTimeout(imageSignatureCleanupTimeout).
		Should(Succeed(), fmt.Sprintf("Failed to remove deployment %q from %q ns", deployName, deployNs))

	klog.V(rdscoreparams.RDSCoreLogLevel).Infof(
		"Deployment %q removed from %q ns", deployName, deployNs)
}
