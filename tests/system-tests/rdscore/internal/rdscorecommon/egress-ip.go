...
func getEgressIPList() ([]string, error) {
	...
	glog.V(rdscoreparams.RDSCoreLogLevel).Infof("EgressIP map: %v", egressIPMap)
...
func verifyEgressIPFailOver(isIPv6 bool) {
	...
	glog.V(rdscoreparams.RDSCoreLogLevel).Infof("Node name where egressIP should be assigned after reboot: %s",
		nodeNameForVerification)
	...
	glog.V(rdscoreparams.RDSCoreLogLevel).Infof("Check %q is assigned to the node %q",
