...
func NewClientBuilderForConfig(config *ranconfig.RANConfig) (*oranapi.ClientBuilder, error) {
	...
	tlsConfig, err := getTLSConfigFromCertificateSecret(config.HubAPIClient, config.O2IMSClientCertSecret)
	if err != nil {
...
func getTLSConfigFromCertificateSecret(hubClient *clients.Settings, certSecretName string) (*tls.Config, error) {
	certSecret, err := secret.Pull(hubClient, certSecretName, ranparam.OCloudOperatorNamespace)
	if err != nil {
		return nil, err
	}
