	...
	It("fails when all matching hardware is already allocated", reportxml.ID("83883"), func() {
		...
		Expect(currentPR.Definition.Status.ProvisioningStatus.ProvisioningPhase).
			To(Equal(provisioningv1alpha1.StateFailed),
				"Expected second ProvisioningRequest to fail when hardware is already allocated")
...
// hardwareProfileForRole returns the HardwareProfile for a node group role, with a single-profile fallback.
...
func waitForPRFulfilledWithMNOExtras(
	...
	hasWorkers, err := helper.HasWorkerNodes()
	Expect(err).ToNot(HaveOccurred(), "Failed to resolve spoke topology for MNO Day2 checks")
	...
	workerCount, err := helper.CountNodesByRole("worker")
	Expect(err).ToNot(HaveOccurred(), "Failed to resolve worker node count for MNO Day2 checks")

	maxUnavailable := getWorkerMaxUnavailable(workerCount)

	groupRoles, err := helper.NodeGroupRolesByName(prBuilder)
	Expect(err).ToNot(HaveOccurred(), "Failed to resolve node group roles from ClusterTemplate")
	...
	err = wait.PollUntilContextTimeout(
		context.TODO(), 10*time.Second, timeout, true, func(ctx context.Context) (bool, error) {
			...
			if phase == provisioningv1alpha1.StateFulfilled &&
				refreshed.Object.Status.ProvisioningStatus.UpdateTime.After(updateTime) {
				return true, nil
			}
...
// getWorkerMaxUnavailable resolves the worker MachineConfigPool maxUnavailable value for rolling-update checks.
func getWorkerMaxUnavailable(workerCount int) int {
	...
	if workerCount > 0 {
		percentString := strings.TrimSuffix(workerMCP.Object.Spec.MaxUnavailable.StrVal, "%")

		percent, err := strconv.Atoi(percentString)
		if err == nil && percent > 0 {
			computedMaxUnavailable := workerCount * percent / 100
			if computedMaxUnavailable < 1 {
				return 1
			}

			return computedMaxUnavailable
		}
	}
...
// observeAllocatedNodeRolling records MNO Day2 rolling-update violations while a ProvisioningRequest is fulfilling.
//
// It inspects only AllocatedNodes referenced by the current PR's InfrastructureResourceStatuses and classifies
// each node by role (resolved from ClusterTemplate nodeGroupData).
//
// Violation types:
//   - ordering: a worker enters an active hardware update (ConfigurationUpdateRequested or InProgress) before every
//     master AllocatedNode reports ConfigurationApplied.
//   - concurrency: more than maxUnavailable worker AllocatedNodes are actively updating at the same time.
//   - gating: an AllocatedNode reports ConfigurationApplied while its backing BMH operationalStatus is not OK, or
//     while the spoke node is not Ready.
func observeAllocatedNodeRolling(
	...
	for _, resourceStatus := range prBuilder.Object.Status.Extensions.InfrastructureResourceStatuses {
		if resourceStatus.ResourceId != "" {
			resourceIDs[resourceStatus.ResourceId] = struct{}{}
		}
	}

	if len(resourceIDs) == 0 {
		klog.V(tsparams.LogLevel).Infof("No infrastructure resource IDs on ProvisioningRequest; skipping MNO observation")
	...
	spokeReady := buildSpokeNodeReadyMap()
	...
	for _, allocatedNode := range allocatedNodes {
		if _, belongsToPR := resourceIDs[allocatedNode.Object.Name]; !belongsToPR {
			continue
		}

		role := allocatedNodeRole(allocatedNode.Object.Spec.GroupName, groupRoles)
...
// buildSpokeNodeReadyMap maps spoke node names to their NodeReady condition state.
func buildSpokeNodeReadyMap() map[string]bool {
...
// allocatedNodeRole resolves an AllocatedNode group name to its node role using ClusterTemplate nodeGroupData.
func allocatedNodeRole(groupName string, groupRoles map[string]string) string {
	if role, ok := groupRoles[groupName]; ok {
		return role
	}
...
// configuredConditionReason returns the Reason from the AllocatedNode Configured condition.
...
// isActiveHardwareUpdate reports whether a Configured condition reason indicates an in-progress hardware update.
...
// verifyConfigAppliedGating checks BMH operational status and spoke node readiness when ConfigApplied is set.
