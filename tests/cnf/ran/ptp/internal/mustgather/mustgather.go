...
func MustGatherIfFailed(
	...
	image, err := getMustGatherImage(client, fallbackImage)
	if err != nil {
...
// getMustGatherImage retrieves the must-gather image from the PTP operator CSV annotation,
// falling back to the provided fallback image if not found.
func getMustGatherImage(client *clients.Settings, fallbackImage string) (string, error) {
	...
	if err != nil {
		...
		return getFallbackImage(fallbackImage)
	}
...
// getFallbackImage returns the fallback image or an error if its empty.
func getFallbackImage(fallbackImage string) (string, error) {
	if fallbackImage == "" {
		return "", fmt.Errorf("no must-gather image available: CSV annotation not found and no fallback configured")
	}
