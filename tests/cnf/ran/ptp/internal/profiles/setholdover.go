...
func ResetHoldOverTimeouts(client *clients.Settings, oldHoldovers HoldOverMap) error {
	...
	for reference, oldHoldover := range oldHoldovers {
		...
		glog.V(tsparams.LogLevel).
			Infof("Resetting holdover timeout for profile %s to %d from %d", reference.ProfileName, oldHoldover,
				ptpConfig.Definition.Spec.Profile[reference.ProfileIndex].PtpClockThreshold.HoldOverTimeout)
