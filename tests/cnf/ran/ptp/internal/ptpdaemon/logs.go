...
// WithIgnoreTimeout sets whether to ignore the timeout if the matcher does not return true. If set to true, the
// [WaitForPodLog] function will not return an error if the timeout is reached, but returns other errors if they occur.
// It defaults to false.
func WithIgnoreTimeout(ignoreTimeout bool) WaitForPodLogOption {
	return func(o *waitForPodLogOptions) {
		o.ignoreTimeout = ignoreTimeout
	}
}
...
// WaitForProfileLoad waits for the profile load message to appear in the PTP daemon pod logs on the specified node.
func WaitForProfileLoad(client *clients.Settings, nodeName string, startTime time.Time, timeout time.Duration) error {
	err := WaitForPodLog(client, nodeName,
		WithMatcher(ContainsMatcher(profileLoadMessage)),
...
// nodes. It returns an error if any of the nodes do not have the profile load message appear in their logs within the
// specified timeout.
func WaitForProfileLoadOnNodes(
	client *clients.Settings, nodeNames []string, startTime time.Time, timeout time.Duration) error {
	if len(nodeNames) == 0 {
		return nil
	}
	...
	for _, nodeName := range nodeNames {
		waitGroup.Go(func() {
			err := WaitForProfileLoad(client, nodeName, startTime, timeout)
			if err != nil {
