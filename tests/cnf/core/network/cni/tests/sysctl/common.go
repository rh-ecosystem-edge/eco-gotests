...
// suite run without skipping when none are found.
...
// ensureSysctlMacVlanSetup returns a worker node and macvlan-capable interfaces for sysctl tests.
...
func countMatchingInterfaceNames(requestedNames, availableUpNames []string) int {
	...
	for _, name := range availableUpNames {
...
func validateSysctlSriovInterfaces(workerNodeList []*nodes.Builder, requestedNumber int) error {
	...
	for _, worker := range workerNodeList {
		...
		availableUpNames := make([]string, len(availableUpSriovInterfaces))
		for i, nic := range availableUpSriovInterfaces {
			availableUpNames[i] = nic.Name
		}

		matchCount := countMatchingInterfaceNames(requestedSriovInterfaceList, availableUpNames)
		if matchCount < requestedNumber {
			return fmt.Errorf("requested interfaces %v are not all present on node %s (found %d of %d)",
				requestedSriovInterfaceList, worker.Definition.Name, matchCount, requestedNumber)
		}
	}
...
func pullSysctlNADConfigRaw(nadName string) string {
	...
	return pulled.Definition.Spec.Config
}

func pullSysctlNADConfigJSON(nadName string) map[string]interface{} {
	rawConfig := pullSysctlNADConfigRaw(nadName)

	var cfg map[string]interface{}
	Expect(json.Unmarshal([]byte(rawConfig), &cfg)).
		To(Succeed(), "Failed to unmarshal NAD %s Spec.Config: %s", nadName, rawConfig)
...
func assertSriovSysctlNADConfig(nadName string, expectedSysctl map[string]string) {
	...
	cfg := pullSysctlNADConfigJSON(nadName)
...
func assertSysctlNADConfigContainsDuplicatedKernelArg(nadName string, sysctlConfig map[string]string) {
	...
	Expect(strings.Count(rawConfig, jsonFragment)).To(Equal(2),
...
func assertSysctlWriteRejectedOnPodInterface(
	...
	for key := range sysctlConfig {
		...
		output, err := runningPod.ExecCommand([]string{
			"sysctl", "-w", fmt.Sprintf("%s=1", sysctlKernelParam),
		})
...
func defineMultiInterfaceClientNetCfg() []*types.NetworkSelectionElement {
	return append(
...
func defineMultiInterfaceServerNetCfg() []*types.NetworkSelectionElement {
...
func defineMultiInterfaceRedirectNetCfg() []*types.NetworkSelectionElement {
	return pod.StaticIPAnnotation(
		tsparams.NetworkWithoutSysctlMutation,
...
func createClientPodWithInit(clientNetCfg []*types.NetworkSelectionElement, initCmd string) *pod.Builder {
	return createSysctlPod(sysctlClientPodName, clientNetCfg, initCmd, []string{"NET_RAW"})
}
