...
var _ = Describe("CNF Sysctl", Ordered, Label(tsparams.LabelSysctlTestCases), ContinueOnFailure, func() {
	...
	Context("pod one secondary interface,", func() {
		It("set accept_redirects=0 on one of two SriovNetworks",
			reportxml.ID("50439"), func() {
				...
				By("Verify all allowed interface sysctls are applied on net1")
		...
		It("sriov-bond interface. One SriovNetwork 2 sriov interfaces, one bound NAD. "+
			"Set accept_redirects=0 on interface",
		...
		It("set accept_redirects=0 on one of two NAD macvlans",
			reportxml.ID("50437"), func() {
				...
				By("Verify manual sysctl -w is rejected inside the running pod")
				assertSysctlWriteRejectedOnPodInterface(
					runningClientPod, tsparams.SingleSysctlFlag, tsparams.MultusFirstInterfaceName)
	...
	Context("pod multiple interfaces,", func() {
		It("set accept_redirects=0 on one first NAD and 1 on the second NAD",
			reportxml.ID("50438"), func() {
				...
				By("Verify all allowed interface sysctls are applied on net2")
				assertSysctlConfiguredOnPodInterface(
					runningClientPod,
					tsparams.AllFlagsSysctlPluginConfig,
					tsparams.MultusSecondInterfaceName)
		...
		It("set accept_redirects=0 on first SriovNetwork and 1 on the second SriovNetwork",
		...
		It("sriov-bond. Set accept_redirects=0 on first bond NAD and 1 on the second bond NAD",
