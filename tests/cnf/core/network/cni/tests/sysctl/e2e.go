...
var _ = Describe("CNF Sysctl", Ordered, Label(tsparams.LabelSysctlTestCases), ContinueOnFailure, func() {
	...
	Context("pod one secondary interface,", func() {
		It("apply full allowed interface sysctl set on one of two SriovNetworks, verify sysctl -n, "+
			...
			reportxml.ID("50439"), func() {
				...
				By("Define and create sr-iov network with full allowed interface sysctl set")
				createSysctlTuningSriovNetwork(
					tsparams.NetworkWithSysctlMutation, tsparams.AllFlagsSysctlPluginConfig, true)
		...
		It("sriov-bond interface with one SriovNetwork and two VF slaves on one bond NAD, "+
			"apply full allowed interface sysctl set on bond0, verify sysctl -n, reject sysctl -w, "+
			"and validate ICMP redirect",
			reportxml.ID("50502"), func() {
				...
				By("Define bond NAD with full allowed interface sysctl set")
				createSysctlBondNad(
					tsparams.NetworkWithSysctlMutation,
					[]string{tsparams.MultusFirstInterfaceName, tsparams.MultusSecondInterfaceName},
					tsparams.AllFlagsSysctlPluginConfig)
				...
				By("Recreate client pod connected to bond NAD with sysctl mutation")
				...
				By("Verify all allowed interface sysctls are applied on bond0")
				assertSysctlConfiguredOnPodInterface(
					runningClientPod,
					tsparams.AllFlagsSysctlPluginConfig,
					tsparams.BondInterfaceName)
		...
		It("apply full allowed interface sysctl set on one of two NAD macvlans, verify sysctl -n, "+
			...
			reportxml.ID("50437"), func() {
				macVlanIf := validMacVlanInterfaces[0].Name
	...
	Context("pod multiple interfaces,", func() {
		It("apply plain macvlan on net1 and full allowed interface sysctl set on net2, "+
			"verify sysctl -n on net2, reject sysctl -w, and validate multi-interface ICMP redirect",
			reportxml.ID("50438"), func() {
				...
				By("Create server and redirect pod")
				createServerPodWithInit(defineMultiInterfaceServerNetCfg(), tsparams.SrvMultiInterfaceInitCMD)
				createRedirectPodWithInit(defineMultiInterfaceRedirectNetCfg(), tsparams.RdrMultiInterfaceInitCMD)

				By("Create client pod connected to both NADs")

				runningClientPod := createClientPodWithInit(
					defineMultiInterfaceClientNetCfg(), tsparams.ClientMultiInterfaceInitCMD)
		...
		It("apply plain SriovNetwork on net1 and full allowed interface sysctl set on net2, "+
			"verify sysctl -n on net2, reject sysctl -w, and validate multi-interface ICMP redirect",
		...
		It("sriov-bond with two bond NADs, apply full allowed interface sysctl set on bond1 only, "+
			"verify sysctl -n, reject sysctl -w, and validate multi-interface ICMP redirect",
			reportxml.ID("50503"), func() {
				...
				By("Create server and redirect pod")
				createServerPodWithInit(defineBondServerNetCfg(true), tsparams.SrvMultiInterfaceInitCMD)
				createRedirectPodWithInit(defineBondRedirectNetCfg(true), tsparams.RdrMultiInterfaceInitCMD)

				By("Create client pod connected to both bond NAD networks")

				runningClientPod := createClientPodWithInit(defineBondClientNetCfg(true), tsparams.ClientMultiInterfaceInitCMD)
				...
				By("Verify all allowed interface sysctls are applied on bond1")
				assertSysctlConfiguredOnPodInterface(
					runningClientPod,
					tsparams.AllFlagsSysctlPluginConfig,
					tsparams.BondInterfaceNameSecond)
