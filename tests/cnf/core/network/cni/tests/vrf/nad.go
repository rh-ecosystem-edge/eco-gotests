...
var _ = Describe("CNF VRF", Ordered, Label(tsparams.LabelNadVRFTestCases), ContinueOnFailure, func() {
	var (
	...
	const (
		redNetName  = "test-vrf-red"
		blueNetName = "test-vrf-blue"
	)
	...
	BeforeAll(func() {
		...
		createVRFNad(blueNetName, sriovInterfacesUnderTest[0], vrfBlueName, nad.IPAMStatic())
		createVRFNad(redNetName, sriovInterfacesUnderTest[0], vrfRedName, nad.IPAMStatic())
	})
	...
	DescribeTable("Integration: NAD, IPAM: static, Interfaces: 1, Scheme: 2 Pods 2 VRFs OCP Primary network overlap",
		...
		func(sameNode bool) {
			...
			runVRFScenario(workerNodeList, sameNode, redNetName, blueNetName, clientNetConfig, serverNetConfig, vrfIPAMStatic)
		},
