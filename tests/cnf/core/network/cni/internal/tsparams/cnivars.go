...
var (
	...
	// SrvLopSecondIPAddr is the second server loopback destination for multi-interface redirect tests.
	...
	// SrvMultiInterfaceInitCMD configures the server pod routing for multi-interface redirect tests.
	SrvMultiInterfaceInitCMD = fmt.Sprintf(
	...
	// RdrMultiInterfaceInitCMD configures the redirect pod routing for multi-interface redirect tests.
	RdrMultiInterfaceInitCMD = fmt.Sprintf("%s && ip route add %s/32 via %s",
	...
	// ClientMultiInterfaceInitCMD configures the client pod routing for multi-interface redirect tests.
	ClientMultiInterfaceInitCMD = fmt.Sprintf("%s && ip route add %s/32 via %s",
