...
// WaitForBondDegradedState polls until the bond is up with exactly one MII-down slave.
// Used when a link is lost via switch port disable and the affected slave is not known.
func WaitForBondDegradedState(clientPod *pod.Builder, bondName string) error {
	deadline := time.Now().Add(BondActiveSlaveChangeTimeout)
	...
	for {
		...
		downCount := countBondSlavesWithMII(slaves, "down")
		upCount := countBondSlavesWithMII(slaves, "up")

		if bondUp && downCount == 1 && upCount >= 1 {
			return nil
		}

		if time.Now().After(deadline) {
			return fmt.Errorf(
				"bond %q did not reach degraded state (one slave MII down) within %v (bondUp=%t, slaves=%v)",
