...
func getIPv4MulticastConfig(mtu int) (group, mac string) {
	if mtu > 1500 {
		return "239.100.100.250", "01:00:5e:64:64:fa"
	}

	return "239.100.0.250", "01:00:5e:64:00:fa"
}
...
func RunTrafficTest(clientPod *pod.Builder, serverIP string, mtu int) error {
	...
	// - IPv4 default uses 239.100.0.250
	// - IPv4 MTU 9000 uses 239.100.100.250
	// - IPv6 uses ff05:5::5 (site-local multicast) for both MTU 1280 and 9000
	multicastGroup := "239.100.0.250"
	if strings.Contains(serverIPAddress, ":") {
		multicastGroup = "ff05:5::5"
	} else if mtu == 9000 {
