...
var (
	...
	// WhereaboutsIPv4Range is the IP range for whereabouts IPAM MTU 500 (full /24 for same broadcast domain).
	// Both client and server use the same /24 range to be on the same broadcast domain.
	WhereaboutsIPv4Range = "192.168.100.0/24"
	// WhereaboutsIPv4Gateway is the gateway for whereabouts IPAM MTU 500 (shared by both client and server).
	WhereaboutsIPv4Gateway = "192.168.100.1"
	// WhereaboutsIPv4RangeMTU9000 is the IP range for whereabouts IPAM MTU 9000 (separate range to avoid conflicts).
	WhereaboutsIPv4RangeMTU9000 = "192.168.101.0/24"
	// WhereaboutsIPv4GatewayMTU9000 is the gateway for whereabouts IPAM MTU 9000.
	WhereaboutsIPv4GatewayMTU9000 = "192.168.101.1"
