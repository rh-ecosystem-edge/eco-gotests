...
var _ = Describe("SR-IOV IPv4", Ordered, Label(tsparams.LabelSuite), ContinueOnFailure, func() {
	...
	BeforeAll(func() {
		...
		err = sriovenv.CreateAllSriovPolicies(sriovInterfacePF1, sriovInterfacePF2, false)
		Expect(err).ToNot(HaveOccurred(), "Failed to create SR-IOV policies")

		By("Waiting for SR-IOV policies to be applied on PF1")
		// Allow extra time for VF creation since policy application can take longer on some clusters
		...
		err = sriovenv.WaitUntilVfsCreated(
			workerNodeList, sriovInterfacePF2, tsparams.TotalVFs, 10*time.Minute)
	...
	Context("Same Node Same PF", func() {
		...
		It("Verify SR-IOV IPv4 connectivity with Whereabouts IPAM, Dynamic MAC, and VLAN",
			reportxml.ID("31808"), func() {
				...
				serverIP, err := sriovenv.GetPodIPFromInterface(vlanServer, tsparams.SecondaryInterfaceName)
	...
	Context("Same Node Different PF", func() {
		...
		It("Verify SR-IOV IPv4 connectivity with Whereabouts IPAM and Dynamic MAC",
			reportxml.ID("31807"), func() {
				...
				By("Getting server IP from pod interface")
				serverIP, err := sriovenv.GetPodIPFromInterface(whereaboutsServer, tsparams.SecondaryInterfaceName)
