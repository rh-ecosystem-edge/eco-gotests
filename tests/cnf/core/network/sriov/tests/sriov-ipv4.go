	...
	Context("Same Node Same PF", func() {
		...
		BeforeAll(func() {
			...
			err = sriovenv.CreateSriovNetworkWithVLANAndWhereabouts(
				...
				tsparams.WhereaboutsIPv4RangeMTU9000, tsparams.WhereaboutsIPv4GatewayMTU9000)
		...
		It("Verify SR-IOV IPv4 connectivity with Whereabouts IPAM, Dynamic MAC, and VLAN",
			reportxml.ID("31808"), func() {
				...
				Eventually(func() error {
					var ipErr error
					serverIPMTU500, ipErr = sriovenv.GetPodIPFromInterface(vlanServerMTU500, tsparams.Net1Interface)
				...
				Eventually(func() error {
					var ipErr error
					serverIPMTU9000, ipErr = sriovenv.GetPodIPFromInterface(vlanServerMTU9000, tsparams.Net1Interface)
	...
	Context("Same Node Different PF", func() {
		...
		It("Verify SR-IOV IPv4 connectivity with Whereabouts IPAM and Dynamic MAC",
			reportxml.ID("31807"), func() {
				...
				Eventually(func() error {
					var ipErr error
					serverIP, ipErr = sriovenv.GetPodIPFromInterface(whereaboutsServer, tsparams.Net1Interface)
...
func createTestServerPod(
	...
	packetSize := packetSizeForMTU(mtu)
	...
	command := buildServerCommand(serverBindIP, interfaceName, ipv4MulticastGroup, packetSize)
