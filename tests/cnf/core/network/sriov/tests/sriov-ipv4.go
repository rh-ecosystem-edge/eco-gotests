...
var _ = Describe("SR-IOV IPv4", Ordered, Label(tsparams.LabelSuite), ContinueOnFailure, func() {
	const (
		...
		clientPodVlan           = "client-vlan"
		serverPodVlan           = "server-vlan"
	...
	Context("Same Node Same PF", func() {
		...
		BeforeAll(func() {
			...
			err = sriovenv.CreateSriovNetworkWithStaticIPAM(
				sriovNetworkSamePFMTU500, sriovResourcePF1MTU500)
			Expect(err).ToNot(HaveOccurred(), "Failed to create SR-IOV network for MTU 500")

			err = sriovenv.CreateSriovNetworkWithStaticIPAM(
				sriovNetworkSamePFMTU9000, sriovResourcePF1MTU9000)
		...
		It("Verify SR-IOV IPv4 connectivity with Static IPAM and Static MAC", reportxml.ID("31801"), func() {
			...
			err = runTrafficTest(clientMTU500, ipaddr.RemovePrefix(tsparams.ServerIPv4IPAddress), mtu500)
			Expect(err).ToNot(HaveOccurred(), "Traffic tests failed for MTU 500")

			By("Running traffic tests with MTU 9000")
			err = runTrafficTest(clientMTU9000, ipaddr.RemovePrefix(tsparams.ServerIPv4IPAddress2), mtu9000)
		...
		It("Verify SR-IOV IPv4 connectivity with Whereabouts IPAM, Dynamic MAC, and VLAN",
			reportxml.ID("31808"), func() {
				...
				var serverIPMTU500 string
				Eventually(func() error {
					var ipErr error
					serverIPMTU500, ipErr = sriovenv.GetPodIPFromInterface(vlanServerMTU500, tsparams.SecondaryInterfaceName)

					return ipErr
				}, tsparams.WaitTimeout, tsparams.RetryInterval).Should(Succeed(),
					"Failed to get server IP from VLAN pod for MTU 500")

				var serverIPMTU9000 string
				Eventually(func() error {
					var ipErr error
					serverIPMTU9000, ipErr = sriovenv.GetPodIPFromInterface(vlanServerMTU9000, tsparams.SecondaryInterfaceName)

					return ipErr
				}, tsparams.WaitTimeout, tsparams.RetryInterval).Should(Succeed(),
					"Failed to get server IP from VLAN pod for MTU 9000")
	...
	Context("Same Node Different PF", func() {
		...
		It("Verify SR-IOV IPv4 connectivity with Whereabouts IPAM and Dynamic MAC",
			reportxml.ID("31807"), func() {
				...
				Eventually(func() error {
					var ipErr error
					serverIP, ipErr = sriovenv.GetPodIPFromInterface(whereaboutsServer, tsparams.SecondaryInterfaceName)

					return ipErr
				}, tsparams.WaitTimeout, tsparams.RetryInterval).Should(Succeed(),
					"Failed to get server IP from whereabouts pod")
	...
	Context("Different Node", func() {
		...
		BeforeAll(func() {
			...
			err = sriovenv.CreateSriovNetworkWithStaticIPAM(
				sriovNetworkDiffNodeMTU500, sriovResourcePF1MTU500)
			Expect(err).ToNot(HaveOccurred(), "Failed to create SR-IOV network for MTU 500")

			err = sriovenv.CreateSriovNetworkWithStaticIPAM(
				sriovNetworkDiffNodeMTU9000, sriovResourcePF1MTU9000)
			Expect(err).ToNot(HaveOccurred(), "Failed to create SR-IOV network for MTU 9000")
...
func createAllSriovPolicies(
	...
	if err := createAndApplySriovPolicy(
...
// createAndApplySriovPolicy creates a single SR-IOV policy and waits for it to be applied.
func createAndApplySriovPolicy(
	name string,
	resourceName string,
	pfName string,
	mtu int,
	...
	return sriovoperator.CreateSriovPolicyAndWaitUntilItsApplied(
		APIClient,
		NetConfig.WorkerLabelEnvVar,
		NetConfig.SriovOperatorNamespace,
		policy,
...
func createTestServerPod(
	...
	if serverBindIP == "" {
		...
		command = []string{"bash", "-c", fmt.Sprintf(
			"sleep 5; "+
				"SERVER_IP=$(ip -4 -o addr show net1 | awk '{print $4}' | cut -d'/' -f1 | head -1); "+
				"echo \"Discovered server IP: $SERVER_IP\"; "+
				"testcmd -listen -protocol tcp -port 5001 -interface net1 -server $SERVER_IP -mtu %d & "+
				...
				"testcmd -listen -multicast -protocol udp -port 5004 -interface net1 -server %s -mtu %d & "+
				"sleep infinity",
			packetSize, packetSize, packetSize, ipv4MulticastGroup, packetSize)}
	} else {
		...
		command = []string{"bash", "-c", fmt.Sprintf(
			"sleep 5; "+
				...
				"testcmd -listen -multicast -protocol udp -port 5004 -interface net1 -server %s -mtu %d & "+
				"sleep infinity",
			tcpBindAddr, packetSize, serverBindIP, packetSize, serverBindIP, packetSize, multicastGroup, packetSize)}
	}
...
func runTrafficTest(clientPod *pod.Builder, serverIP string, mtu int) error {
	...
	if err := cmd.ICMPConnectivityCheck(clientPod, []string{serverIPWithPrefix}); err != nil {
	...
	if err := runProtocolTest(clientPod, "TCP",
		fmt.Sprintf("testcmd -protocol tcp -port 5001 -interface net1 -server %s -mtu %d",
			serverIPAddress, packetSize)); err != nil {
	...
	// Multicast group based on IP version and MTU.
	...
	// Delay to ensure server's multicast listener is ready.
	time.Sleep(30 * time.Second)

	if err := runProtocolTest(clientPod, "multicast",
		fmt.Sprintf("testcmd -multicast -protocol udp -port 5004 -interface net1 -server %s -mtu %d",
			multicastGroup, packetSize)); err != nil {
