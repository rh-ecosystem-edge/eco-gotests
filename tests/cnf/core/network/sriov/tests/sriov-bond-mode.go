...
const (
	...
	// bondMinVFsDualStackMLX is the minimum total VFs per PF for dual-stack bond tests on
	// Mellanox and other devices that expose five VFs.
	bondMinVFsDualStackMLX = 5
	// bondMinVFsDualStackCompact is the minimum total VFs per PF for the compact dual-stack layout.
	bondMinVFsDualStackCompact = 10
	// bondMinVFsDualStackExpanded is the minimum total VFs per PF for the expanded dual-stack layout.
	bondMinVFsDualStackExpanded = 20
...
var _ = Describe(
	...
	func() {
		...
		BeforeAll(func() {
			...
			bondIPv6Supported = supportsIPv6
			...
			if supportsIPv4 && supportsIPv6 {
				...
				if minTotal < bondMinVFsDualStackMLX {
					Skip(fmt.Sprintf(
						"Dual-stack bond tests require >=%d VFs per PF on every worker; min across workers: pf1=%d, pf2=%d",
						bondMinVFsDualStackMLX, pf1Total, pf2Total))
				}

				var layoutOK bool

				vf4SmallStart, vf4SmallEnd, vf4LargeStart, vf4LargeEnd,
					vf6SmallStart, vf6SmallEnd, vf6LargeStart, vf6LargeEnd, layoutOK =
					selectBondDualStackVFLayout(pf1Total, pf2Total)
				Expect(layoutOK).To(BeTrue(), "Failed to select dual-stack bond VF layout")

				if minTotal < bondMinVFsDualStackCompact {
					// Five-VF Mellanox PFs cannot fit non-overlapping IPv4+IPv6 bond policy ranges;
					// run IPv4 bond tests only on dual-stack clusters with limited VF counts.
					bondIPv6Supported = false
				}
			}
			...
			if supportsIPv4 {
				setupBondStackForFamily(bondStackFamilyParams{
					...
					networks: bondSlaveNetworkConfigs(
		...
		Context("Mode: active-active", func() {
			...
			FIt("balance-rr DiffNodeDiffPF IPv4", reportxml.ID("89052"), func() {
				if !netenv.ClusterSupportsIPv4(clusterIPFamily) {
					Skip("Cluster does not support IPv4 - skipping SR-IOV bond IPv4 tests")
				}
		...
		Context("Scale: Bond with 16 VFs", func() {
			...
			BeforeAll(func() {
				...
				if supportsIPv4 {
					...
					err = sriovenv.CreateSriovBondNetwork(scaleNetAIPv4, scaleResAIPv4)
					Expect(err).ToNot(HaveOccurred(), "Failed to create IPv4 scale SR-IOV network A")

					err = sriovenv.CreateSriovBondNetwork(scaleNetBIPv4, scaleResBIPv4)
				...
				if supportsIPv6 {
					...
					err = sriovenv.CreateSriovBondNetwork(scaleNetAIPv6, scaleResAIPv6)
					Expect(err).ToNot(HaveOccurred(), "Failed to create IPv6 scale SR-IOV network A")

					err = sriovenv.CreateSriovBondNetwork(scaleNetBIPv6, scaleResBIPv6)
...
// selectBondDualStackVFLayout returns non-overlapping VF ranges for IPv4 and IPv6 bond policies.
func selectBondDualStackVFLayout(pf1Total, pf2Total int) (
	vf4SmallStart, vf4SmallEnd, vf4LargeStart, vf4LargeEnd int,
	vf6SmallStart, vf6SmallEnd, vf6LargeStart, vf6LargeEnd int,
	ok bool,
) {
	minTotal := pf1Total
	if pf2Total < minTotal {
	...
	switch {
	case minTotal >= bondMinVFsDualStackExpanded:
		// Expanded layout: IPv4 VFs 0-9, IPv6 VFs 10-19.
		return 0, 4, 5, 9, 10, 14, 15, 19, true
	case minTotal >= bondMinVFsDualStackCompact:
		// Compact layout for 10-VF PFs: IPv4 VFs 0-4, IPv6 VFs 5-9 (split 2+3 per MTU tier).
		return 0, 1, 2, 4, 5, 6, 7, 9, true
	default:
		// MLX layout for 5-VF PFs: IPv4 VFs 0-1 (small) and 2-4 (large).
		return 0, 1, 2, 4, 0, 0, 0, 0, true
	}
}
...
func setupBondStackForFamily(params bondStackFamilyParams) {
	...
	if params.dualStack {
		policies := []struct {
			...
			{"pf1", params.resPF1Small, params.pf1, params.pf1NumVFs, params.mtuSmall, params.vfSmallStart, params.vfSmallEnd},
			{"pf1", params.resPF1Large, params.pf1, params.pf1NumVFs, params.mtuLarge, params.vfLargeStart, params.vfLargeEnd},
			{"pf2", params.resPF2Small, params.pf2, params.pf2NumVFs, params.mtuSmall, params.vfSmallStart, params.vfSmallEnd},
			{"pf2", params.resPF2Large, params.pf2, params.pf2NumVFs, params.mtuLarge, params.vfLargeStart, params.vfLargeEnd},
		}
