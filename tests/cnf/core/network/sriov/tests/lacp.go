...
var _ = Describe("LACP Status Relay ", Ordered, Label(tsparams.LabelSuite), ContinueOnFailure, func() {
	...
	BeforeAll(func() {
		...
		if len(switchInterfaces) >= 2 {
			...
			firstTwoSwitchInterfaces = switchInterfaces
		}
	...
	Context("linux pod", func() {
		...
		It("Verify bond active-backup recovery when PF LACP failure disables VF", reportxml.ID("83319"), func() {
			...
			performLACPFailureAndRecoveryTest(bondedClientPod, worker0NodeName, primaryInterface,
...
// DefineBondNad returns network attachment definition for a Bond interface using eco-goinfra.
