	...
	Context("linux client", func() {
		BeforeAll(func() {
			...
			_, err = sriov.NewPolicyBuilder(
				...
				5,
				[]string{fmt.Sprintf("%s#0-4", srIovInterfacesUnderTest[1])},
				nodeSelectorWorker0).WithMTU(9000).WithVhostNet(true).Create()
		...
		FIt("Verify that a Linux pod with an active-backup bonded interface fails over when the associated VF is "+
			...
			bondedClientPod, err := createBondedClient(bondedClientPodName, workerNodeList[0].Definition.Name)
			Expect(err).ToNot(HaveOccurred(), "Failed to create bonded client pod")

			verifyPFLACPMonitorLogs(workerNodeList[0].Definition.Name, srIovInterfacesUnderTest, logTypeInitialization, 0, "")

			By("Test bonded interface connectivity using testcmd")
			validateBondedTCPTraffic(bondedClientPod, testInterface, testClientIP)
			...
			verifyPFLACPMonitorLogs(workerNodeList[0].Definition.Name, srIovInterfacesUnderTest, logTypeVFDisable,
				5, srIovInterfacesUnderTest[0])
...
func configureLACPBondInterfaces(workerNodeName string, sriovInterfacesUnderTest []string) error {
	...
	bond10Policy := nmstate.NewPolicyBuilder(APIClient, "bond10", nodeSelector).
		WithBondInterface([]string{sriovInterfacesUnderTest[0]}, "bond10", "802.3ad", bondInterfaceOptions)
	...
	if len(sriovInterfacesUnderTest) > 1 {
		bond20Policy := nmstate.NewPolicyBuilder(APIClient, "bond20", nodeSelector).
			WithBondInterface([]string{sriovInterfacesUnderTest[1]}, "bond20", "802.3ad", bondInterfaceOptions)
...
func setLACPBlockFilterOnInterface(credentials *sriovenv.SwitchCredentials, enable bool) {
	...
	err = jnpr.Config(commands)
	Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("Failed to %s LACP block filter on interface", strings.ToLower(actionDescription)))

	By(fmt.Sprintf("Successfully %s LACP block filter on interface %s", strings.ToLower(actionDescription), lacpInterface1))
}
...
func verifyPFStatusRelayOperatorRunning() error {
	...
	pfStatusOperatorDeployment, err := deployment.Pull(APIClient, "pf-status-relay-operator-controller-manager", "openshift-pf-status-relay-operator")
	if err != nil {
...
func validateBondedTCPTraffic(clientPod *pod.Builder, interfaceName, serverIP string) {
	By(fmt.Sprintf("Validating TCP traffic from %s to %s via interface %s",
		clientPod.Definition.Name, serverIP, interfaceName))
	...
	Expect(output.String()).Should(ContainSubstring("0 packet loss"),
		fmt.Sprintf("Bonded interface %s should have 0 packet loss", interfaceName))
...
func verifyPFLACPMonitorLogs(nodeName string, srIovInterfacesUnderTest []string, logType string, expectedVFs int, targetInterface string) {
	...
	default:
		Expect(false).To(BeTrue(), fmt.Sprintf("Invalid logType '%s'. Use '%s' or '%s'", logType, logTypeInitialization, logTypeVFDisable))
	}
}
...
func verifyVFDisableLogs(podLogs string, targetInterface string, expectedVFs int) {
	...
	for vfID := 0; vfID < expectedVFs; vfID++ {
		expectedLogEntry := fmt.Sprintf(`"vf link state was set","id":%d,"state":"disable","interface":"%s"`, vfID, targetInterface)
