...
func setLACPBlockFilterOnInterface(credentials *sriovenv.SwitchCredentials, enable bool) {
	...
	// Wait for LACP failover to complete when enabling the block filter
	if enable {
		By("Waiting for LACP failover to complete")
...
func getPFLACPMonitorPod(nodeName string) (*pod.Builder, error) {
	...
	namespace := NetConfig.PFStatusRelayOperatorNamespace
