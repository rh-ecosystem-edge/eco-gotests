	...
	Context("IBGP Single hop", func() {
		...
		It("Verify that when the allow all mode is configured all routes are received on the FRR speaker",
			reportxml.ID("74273"), func() {
				...
				By("Creating BGP Peers")
				createOVNKCompatibleBGPPeer(tsparams.BgpPeerName1, ipv4metalLbIPList[0], tsparams.LocalBGPASN)
...
func createOVNKCompatibleBGPPeer(name, peerIP string, remoteAsn uint32) {
...
func createOVNKCompatibleFRRConfiguration(name, bgpPeerIP string, remoteAS uint32) {
	...
	//nolint:staticcheck // DisableMP is deprecated but required for RouteAdvertisement compatibility
