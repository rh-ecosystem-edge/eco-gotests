	...
	Context("OVN-K RouteAdvertisements", func() {
		...
		It("Verify OVN-K RouteAdvertisement advertises pod network routes to external FRR",
			reportxml.ID("75001"), func() {
				...
				createOVNKRouteAdvertisementAndWaitUntilReady("default")
...
func enableOVNKRouteAdvertisements() {
	By("Patching network operator to enable RouteAdvertisements")

	patchData := []byte(`{"spec":{"defaultNetwork":{"ovnKubernetesConfig":{"routeAdvertisements":"Enabled"}}}}`)

	err := APIClient.Patch(context.TODO(), &operatorv1.Network{
		ObjectMeta: metav1.ObjectMeta{
			Name: "cluster",
		},
	}, client.RawPatch(apimachinerytype.MergePatchType, patchData))
	Expect(err).ToNot(HaveOccurred(), "Failed to enable RouteAdvertisements on network operator")

	By("Waiting for OVN-K to reconcile RouteAdvertisements setting")
...
func disableOVNKRouteAdvertisements() {
	By("Patching network operator to disable RouteAdvertisements")

	patchData := []byte(`{"spec":{"defaultNetwork":{"ovnKubernetesConfig":{"routeAdvertisements":"Disabled"}}}}`)
...
// cleanupRouteAdvertisements removes all RouteAdvertisement resources from the cluster.
func cleanupRouteAdvertisements() {
	By("Cleaning up RouteAdvertisement resources")

	raList, err := ovn.ListRouteAdvertisements(APIClient)
	if err != nil {
		By(fmt.Sprintf("Failed to list RouteAdvertisements: %v", err))
	...
	for _, ra := range raList {
		err := ra.Delete()
		if err != nil {
			By(fmt.Sprintf("Failed to delete RouteAdvertisement %s: %v", ra.Definition.Name, err))
		}
	}
}
