...
var _ = Describe(
	...
	func() {
		...
		It("performs disconnected IBI cluster node preinstall end to end", reportxml.ID("89666"), func() {
			...
			if seedVersion == "" {
				seedVersion = lca.SeedVersionFromSeedImage(raninittools.RANConfig.SeedImage)
			}

			ibiData := lca.InstallationConfigInput{
			...
			err = lca.WriteInstallationConfig(ibiData, workDir)
