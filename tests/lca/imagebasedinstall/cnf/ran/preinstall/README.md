...
- `internal/helpers` — suite-specific utilities:
  ...
  - `image-based-installation-config.yaml` via eco-goinfra `pkg/lca` (installer-compatible schema, assisted `NetConfig` from `pkg/schemes/assisted`)
...
4. **Network Configuration** — From `spec.nodes[0].nodeNetwork.config` (eco-goinfra assisted `NetConfig` in the installer manifest)
...
2. **Test 1**: Generate IBI ISO
   ...
   - Generate `image-based-installation-config.yaml` via eco-goinfra `lca.WriteInstallationConfig`
