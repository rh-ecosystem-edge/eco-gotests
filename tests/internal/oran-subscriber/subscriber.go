...
func PullPod(client *clients.Settings, namespace string) (*pod.Builder, error) {
	matchingPods, err := pod.List(client, namespace, metav1.ListOptions{
	...
	if len(matchingPods) == 0 {
		return nil, fmt.Errorf("no pods for the subscriber found in namespace %q", namespace)
	}
